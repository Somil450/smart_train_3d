...
#include <OneWire.h>            // Install: "OneWire"
#include <DallasTemperature.h>  // Install: "DallasTemperature"
#include <TinyGPS++.h>          // Install: "TinyGPSPlus"
...
float motor2Temp = 0.0;
bool motor2Overheated = false;
...
void broadcastSensorData() {
  ...
  if (motor2Temp > 80.0) tempStatus = "CRITICAL";
  else if (motor2Temp >= 50.0) tempStatus = "DANGER";
  ...
  json += "\"m2_t\":" + String(motor2Temp, 1) + ",";
  json += "\"m2_ts\":\"" + tempStatus + "\",";
  json += "\"lat\":" + String(gpsLat, 6) + ",";
  json += "\"lng\":" + String(gpsLng, 6) + ",";
  json += "\"sats\":" + String(gpsSats) + ",";
  json += "\"v_thresh\":" + String(VIBRATION_THRESHOLD, 3);
  json += "}";
...
void receiveData(
  ...
  else if (command == 'C' || command == 'c') { motor1Stop();  if (!motor2Overheated) { motor2Forward(); Serial.println("MOTOR 2 FORWARD"); } else { motor2Stop(); Serial.println("MOTOR 2 BLOCKED - TEMP > 80"); } }
  else if (command == 'D' || command == 'd') { motor1Stop();  if (!motor2Overheated) { motor2Backward(); Serial.println("MOTOR 2 BACKWARD"); } else { motor2Stop(); Serial.println("MOTOR 2 BLOCKED - TEMP > 80"); } }
...
void loop() {
  ...
  // Non-blocking temperature reading (every 2 seconds)
  ...
  if (tempRequested && millis() - lastTempRequest >= 750) { // 750ms is enough for DS18B20 12-bit conversion
    motor2Temp = sensors.getTempCByIndex(0);
    tempRequested = false;
    lastTempRequest = millis(); // Reset timer so it waits another 2s from now
    
    // Safety cutoff logic
    if (motor2Temp > 80.0) {
      if (!motor2Overheated) {
        Serial.println("CRITICAL: Motor 2 Temp > 80! Stopping Motor 2.");
      }
      motor2Overheated = true;
      motor2Stop(); // Force stop immediately if it was running
    } else {
      if (motor2Overheated) {
        Serial.println("RECOVERY: Motor 2 Temp <= 80. Resuming control.");
      }
      motor2Overheated = false;
    }
  }

  // Communication Safety Timeout: Stop motors if no command received for 500ms
  if (millis() - lastCommandTime > 500) {
    if (motor1Active || motor2Active) {
      Serial.println("Connection lost or no commands! Stopping all motors.");
      stopAllMotors();
    }
  }
  ...
  if (millis() - lastPublish >= 500) {
    ...
    Serial.print(" | Vib2="); Serial.print(lastVib2, 3);
    Serial.print(" | Temp2="); Serial.print(motor2Temp, 2);
    Serial.print(" | GPS="); 
